...
# Funciones para generar dimensiones
def dim_cliente():
    return pd.DataFrame({
...
@app.get("/")
def root():
    return {"mensaje": "API CSV modelo estrella telecomunicaciones"}

@app.get("/csv/{tabla}")
def generar_csv(tabla: str):
    funciones = {
        'dim_cliente': dim_cliente,
        'dim_plan': dim_plan,
        'dim_dispositivo': dim_dispositivo,
        'dim_ubicacion': dim_ubicacion,
        'dim_facturacion': dim_facturacion,
        'dim_promocion': dim_promocion,
        'dim_fecha': dim_fecha,
        'dim_soporte': dim_soporte,
        'dim_servicio': dim_servicio,
        'dim_evento': dim_evento,
        'hechos_telecom': hechos_telecom,
    }
    if tabla not in funciones:
        return {"error": f"Tabla '{tabla}' no reconocida."}
